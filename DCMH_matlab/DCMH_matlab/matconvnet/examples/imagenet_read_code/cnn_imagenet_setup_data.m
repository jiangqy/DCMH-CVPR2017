...
names = {} ;
labels = {} ;
for d = dir(fullfile(opts.dataDir, 'train', 'n*'))'
  [~,lab] = ismember(d.name, cats) ;
  ims = dir(fullfile(opts.dataDir,  'train', d.name, '*.JPEG')) ;
...
ims = dir(fullfile(opts.dataDir,  'val', '*.JPEG')) ;
names = sort({ims.name}) ;
...
ims = dir(fullfile(opts.dataDir, 'test', '*.JPEG')) ;
names = sort({ims.name}) ;
